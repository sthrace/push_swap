...
static void	ft_a_to_b(t_array *data)
{
	...
	if (data->size_b > 2 && data->stack_b[0] == ft_minmax(data->stack_b, data->size_b, 10, -1))
		ft_rotate(data, 'b', 1);
	else if (data->size_b > 1 && data->stack_b[1] > data->stack_b[0])
	{
		if (data->size_a > 1 && data->stack_a[0] > data->stack_a[1])
			ft_swap(data, 's', 1);
		else
			ft_swap(data, 'b', 1);
	}
	...
	if (!ft_issorted(data->stack_a, data->size_a, 1))
	{
		while (k < 50)
		{
			...
			if (i < j + 1)
			{
				...
				if (data->stack_a[0] == ft_minmax(data->stack_b, data->size_b, 20, -1))
					ft_push(data, 'b', 1);
				else
					ft_a_to_b(data);
				k += 1;
